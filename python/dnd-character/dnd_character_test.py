...
from dnd_character import Dice, DiceValues, Character, modifier

# Tests adapted from `problem-specifications//canonical-data.json`


from random import Random
random = Random(123) # seed
class DndDiceTest(unittest.TestCase):
    def test_dice_roll(self):
        dice = Dice(12, random)
        self.assertEqual(dice.roll(10).values, [1, 5, 2, 7, 5, 2, 1, 7, 9, 9])

class DndDiceValuesTest(unittest.TestCase):
    def test_dice_values_top(self):
        diceValues = DiceValues([5, 7, 2, 2, 1, 7, 4, 4, 12, 11, 9, 2])
        self.assertEqual(diceValues.top(5).values, [12, 11, 9, 7, 7])

    def test_dice_values_sum(self):
        diceValues = DiceValues([5, 7, 2, 2, 1, 7, 4, 4, 12, 11, 9, 2])
        self.assertEqual(diceValues.top(5).sum(), 46)
